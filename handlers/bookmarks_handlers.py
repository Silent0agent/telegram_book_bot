...
from services.database_services import (
    sqlite_get_bookmarks_with_books_by_user_id,
)
...
async def process_choose_bookmarks_operation(
    callback: CallbackQuery,
    session: AsyncSession,
):
    await callback.answer()
    if callback.data == "edit_bookmarks":
        bookmarks = await sqlite_get_bookmarks_with_books_by_user_id(
            session,
...
async def process_delete_bookmark(
    callback: CallbackQuery,
    session: AsyncSession,
):
    await callback.answer()
