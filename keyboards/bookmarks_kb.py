...
def create_bookmarks_keyboard(*bookmarks: Bookmark) -> InlineKeyboardMarkup:
    ...
    page_label = LEXICON["bookmark_page_label"]
    ...
    for bookmark in sorted(
        ...
        kb_builder.row(
            InlineKeyboardButton(
                text=f"{bookmark.book.title} - {page_label} "
                f"{bookmark.page_number} - {bookmark.note}",
...
def create_edit_keyboard(*bookmarks: Bookmark) -> InlineKeyboardMarkup:
    ...
    kb_builder = InlineKeyboardBuilder()
    page_label = LEXICON["bookmark_page_label"]
    del_label = LEXICON["del"]
    # Наполняем клавиатуру кнопками-закладками в порядке возрастания
    for bookmark in bookmarks:
        kb_builder.row(
            InlineKeyboardButton(
                text=f"{del_label} {bookmark.book.title} - {page_label} "
                f"{bookmark.page_number} - {bookmark.note}",
