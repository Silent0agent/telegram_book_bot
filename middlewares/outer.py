...
class StateValidationMiddleware(BaseMiddleware):
    def __init__(self):
        ...
        self.process_config = {
            "book_adding": {
                ...
                "warning": LEXICON["cancel_add_book_first_warning"],
            },
            "review_adding": {
                ...
                "warning": LEXICON["cancel_add_review_first_warning"],
            },
        }
    ...
    async def __call__(
        ...
        for _, config in self.process_config.items():
            if current_state in [
                s.state for s in config["state_group"].__all_states__
            ]:
                ...
                if not await self._is_message_valid(
                    event,
                    state,
                    config["data_key"],
                ):
                    await event.answer(
                        LEXICON["old_message_alert"],
                        show_alert=True,
                    )
                    return None
    ...
    def _is_allowed_action(
        ...
        return any(
            action_data.startswith(action) for action in allowed_actions
        )

    async def _is_message_valid(
        self,
        event: CallbackQuery,
        state: FSMContext,
        data_key: str,
...
# Обработка ввода команд, сбрасывающих состояние
# Данные о текущей книге и текущих результатах поиска не сбрасываются
        ...
        if any(event.text.startswith(cmd) for cmd in reset_commands):
            ...
            for state_group, keys in cleanup_map.items():
                if current_state and current_state.startswith(state_group):
                    ...
                    new_data = {
                        k: v for k, v in current_data.items() if k not in keys
                    }
    ...
    async def __call__(
        ...
        if current_state and current_state.startswith("FSMSearchBook"):
            ...
            if isinstance(event, Message):
                ...
                if (
                    not event.text
                ):  # Если сообщение не текстовое (фото, стикер и т.д.)
                    await self._reset_search_by_genres(state)
                    await event.answer(
                        LEXICON["search_message_type_error"]
                        + "\n"
                        + LEXICON["search_stop"],
                    )
            ...
            # Если это callback (нажатие кнопки)
                ...
                if not any(
                    event.data.startswith(action) for action in allowed_actions
                ):
                    await self._reset_search_by_genres(state)
                    await event.answer(
                        LEXICON["search_stop"],
